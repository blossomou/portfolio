...
const Sun = () => {
  return (
    <div>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 25" width="35" height="35">
        <path
          style={{ fill: '#DA95D3' }}
...
const ThemeToggle = () => {
  ...
  return (
    <div
      className="flex items-center justify-between px-2 py-1 border border-solid cursor-pointer border-primarycolor rounded-xl"
      onClick={buttonClick}
    >
      <div className={(theme === Theme.LIGHT ? 'opacity-100' : 'opacity-10 hover:opacity-50').concat(' mr-3')}>
        <Sun />
      </div>
      <div className={theme === Theme.DARK ? 'opacity-100' : 'opacity-30 hover:opacity-60'}>
        <Moon />
      </div>
    </div>
  );
